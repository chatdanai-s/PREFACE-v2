...
# Moon background handling modules are lazy imported.
...
def getFilterParams(csvcorepath, Filter):
    df_filters = pd.read_csv(rf'{csvcorepath}/filter_information.csv')
    row_used = df_filters.loc[df_filters['filter'] == Filter].iloc[0]
    effective_wavelength = float(row_used['effective_wavelength'])
    zeropoint_irradiance = float(row_used['f_lamdba_zeropoint_e-11'])
    allen_full_moon_mag = float(row_used['allen_fullmag'])  # Sampled from LIME during Oct 2025 - May 2026 for TNT ULTRASPEC
...
# Interpolation for altaz
def interpolate_altaz(Obstimes, altazs, new_times):
    alt_fn = interp1d(Obstimes.jd, altazs.alt.deg, kind='cubic')
    az_fn = interp1d(Obstimes.jd, altazs.az.deg, kind='cubic')
    alt_interp = alt_fn(new_times.jd)
...
def download_IERS(csvcorepath):
    IERS_A_URL = 'https://datacenter.iers.org/data/9/finals2000A.all'
    IERS_PATH_FILENAME = 'IERSPath.txt'
    ...
    if data.is_url_in_cache(IERS_A_URL):
        ...
        with open(rf'{csvcorepath}/{IERS_PATH_FILENAME}', 'w') as IERS_File:    # Write location and new hash to .txt file for future ref.
            c = IERS_File.write(str(data.download_file(IERS_A_URL, cache=True, show_progress=True)))

    else:
        print('[MP_Wrapper_Datacenter] Retrieving IERS table from mirror...')
        try:    # Try/except blocks are needed in case of bad internet connection. #JustEduroamThings
            ...
            with open(rf'{csvcorepath}/{IERS_PATH_FILENAME}', 'w') as IERS_File:
    ...
    with open(rf'{csvcorepath}/{IERS_PATH_FILENAME}', 'r') as IERS_File:
       c = IERS_File.read().replace('\n','')        # Reads name of cached IERS table.

    A = creation_date(rf'{csvcorepath}/{IERS_PATH_FILENAME}') 
    LegA = dt.datetime.fromtimestamp(A)             # Converts date to human-readable timestamp.

    if dt.datetime.now() - LegA >= dt.timedelta(days=7):  # If old, we update.
        def retrieveIERS():
        ...
        print('[MP_Wrapper_Datacenter] Retrieving updated IERS table: please stand by...')
        try:
            retrieveIERS()
        except urllib.error.URLError:
            print('>> The request timed out; trying again...')
        ...
        print('[MP_Wrapper_Datacenter] IERS table is up to date.')

# Make directory for output graphs, if it doesn't exist already.
def make_OutputGraphDir(csvoutpath, Inst, Filter, ObsStart, ObsEnd):
    graph_folder = rf'{csvoutpath}/{Inst}_{Filter}-band_{ObsStart.strftime("%b %d %Y")}_to_{ObsEnd.strftime("%b %d %Y")}/' # ADDED 
    if os.path.exists(graph_folder):
        shutil.rmtree(graph_folder)     # Reset entire image folder if exists
...
def get_LocAndTimezoneStr(ds, S):
    Loc = EarthLocation(lat=ds['Lat'][S] * u.deg,
                        lon=ds['Long'][S] * u.deg,
                        height=ds['Alt'][S] * u.m)  # Location info of telescope
    tf = TimezoneFinder()
...
# First create a table with 5-min precision, then linearly interpolate for 1-min precision.
def make_LUT_AltAzs(csvcorepath, Inst, ObsStart, ObsEnd, Loc):
    LUT_FILEPATH_ALTAZS = rf'{csvcorepath}/Lookup_tables/sun_moon_altazs_at_{Inst}_{ObsStart.strftime("%b %d %Y")}_to_{ObsEnd.strftime("%b %d %Y")}.ecsv'
    if not os.path.exists(LUT_FILEPATH_ALTAZS):
        print('[MP_Wrapper_Datacenter] Creating lookup table for Sun and Moon position in local observer frame - This may take a while!')
        ObsStart_LUT = Time(ObsStart - dt.timedelta(days=2), format='datetime', scale='tdb')
        ObsEnd_LUT = Time(ObsEnd + dt.timedelta(days=2), format='datetime', scale='tdb')

        timestep = TimeDelta(300, format='sec')   # 5-minute sampling
        n_steps_5m = int(((ObsEnd_LUT - ObsStart_LUT) / timestep).value) + 1
        Obstimes_5m = ObsStart_LUT + timestep * np.arange(n_steps_5m)

        timestep = TimeDelta(60, format='sec')   # 1-minute sampling
        n_steps_1m = int(((ObsEnd_LUT - ObsStart_LUT) / timestep).value) + 1
        Obstimes_1m = ObsStart_LUT + timestep * np.arange(n_steps_1m)
        ...
        print(f'>> AltAz pairs created for Sun. ({time.time()-start:.2f} s)')
        ...
        print(f'>> AltAz pairs created for Moon. ({time.time()-start:.2f} s)')
        ...
        t = QTable([Obstimes_1m, sun_alt, sun_az, moon_alt, moon_az], names=tcols)
        for col in tcols[1:]:  # Reduce to 5 decimal points
            t[col] = np.round(t[col], 5)
        t.write(LUT_FILEPATH_ALTAZS, overwrite=True)

        print(f'>> AltAz LUT exported to {LUT_FILEPATH_ALTAZS}. ({time.time()-start:.2f} s)')
    else:
        print('[MP_Wrapper_Datacenter] Sun-moon AltAz LUT already exists.')
...
def P2Wrap(csvcorepath, csvinpath, csvoutpath,
           ...
           EventPercent_minThreshold, ObsStart, ObsEnd, ncores):
    print('[MP_Wrapper_Datacenter] Pre-multiprocessing procedures initiating...')

    download_IERS(csvcorepath)
    make_OutputGraphDir(csvoutpath, Inst, Filter, ObsStart, ObsEnd)
    Loc, latvalue, lonvalue, altvalue, timezone_str = get_LocAndTimezoneStr(ds, S)
    LUT_FILEPATH_ALTAZS = make_LUT_AltAzs(csvcorepath, Inst, ObsStart, ObsEnd, Loc)
    make_LUT_Moon(csvcorepath, Inst, ObsStart, ObsEnd, Moon_Noise,
                  latvalue, lonvalue, altvalue, LUT_FILEPATH_ALTAZS) 

    # Multiprocessing start
    print(f'+++ Phase Two: Multiprocessing engaged for {ncores} cores. +++')
    ...
    input_pattern_with_path = os.path.normpath(os.path.join(csvinpath, filename_pattern))
    jobs = glob.glob(input_pattern_with_path)   # Our multiprocesses will live here.
    ...
    with parallel_config(backend='loky', prefer='processes', inner_max_num_threads=1):
        for _ in Parallel(n_jobs=ncores, pre_dispatch=4*ncores, return_as='generator_unordered')(
            delayed(P2MP_Process.P2Predictor)(csvcorepath, csv_in, csvoutpath,
                                              ...
                                              Loc, timezone_str) for csv_in in tqdm(jobs, desc="CSVs initiated")):
            pass
